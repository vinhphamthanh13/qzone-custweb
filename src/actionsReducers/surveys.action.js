...
export const setSurveysApi = headers => async (dispatch) => {
...
export const setSurveyById = id => async (dispatch) => {
  dispatch(setLoading(true));
  const [result, error] = await handleRequest(getSurveyByIdApi, [id]);
  if (error) {
    dispatch(setError(error));
  } else {
